...
public class WeatherStationActivity extends Activity {
    ...
    private float mLastFTemperature;
    ...
    private SensorEventListener mTemperatureListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            mLastFTemperature = 9 * mLastTemperature / 5 + 32;
            Log.d(TAG, "sensor changed: " + mLastTemperature);
            if (mDisplayMode == DisplayMode.TEMPERATURE) {
                updateDisplay(mLastTemperature);
            } else  if (mDisplayMode == DisplayMode.FTEMPERATURE) {
                        updateDisplay(mLastFTemperature);
        }
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            mButtonInputDriver1 = new ButtonInputDriver(BoardDefaults.getButtonGpioPin1(),
            ...
            mButtonInputDriver2 = new ButtonInputDriver(BoardDefaults.getButtonGpioPin2(),
	    ...
	    mButtonInputDriver3 = new ButtonInputDriver(BoardDefaults.getButtonGpioPin3(),
        ...
        try {
            ...
            mLed1.setEdgeTriggerType(Gpio.EDGE_NONE);
            mLed1.setDirection(Gpio.DIRECTION_OUT_INITIALLY_LOW);
            mLed1.setActiveType(Gpio.ACTIVE_HIGH);
            ...
            mLed3.setEdgeTriggerType(Gpio.EDGE_NONE);
            mLed3.setDirection(Gpio.DIRECTION_OUT_INITIALLY_LOW);
            mLed3.setActiveType(Gpio.ACTIVE_HIGH);
        } catch (IOException e) {
	...
	if (keyCode == KeyEvent.KEYCODE_B) {
            mDisplayMode = DisplayMode.FTEMPERATURE;
            updateDisplay(mLastFTemperature);
            try {
                mLed2.setValue(true);
            } catch (IOException e) {
       ...
       if (keyCode == KeyEvent.KEYCODE_C) {
            mDisplayMode = DisplayMode.MESSAGE;
            updateBarometer(1035);
            updateMessage("TEST");
            try {
                mLed3.setValue(true);
            } catch (IOException e) {
	...
	if (mButtonInputDriver2 != null) {
            try {
                mButtonInputDriver2.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            mButtonInputDriver2 = null;
        }

        if (mButtonInputDriver3 != null) {
            try {
                mButtonInputDriver3.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            mButtonInputDriver3 = null;
        }
